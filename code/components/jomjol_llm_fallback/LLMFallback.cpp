                          ...
                          const std::string& context)
{
    ...
    std::string url;
    std::string body;
    const std::string* apiKey     = nullptr;
    ...
    if (s_cfg.provider == LLMProvider::OpenAI) {
        ...
        body = buildOpenAIBody(s_cfg.model, b64Buf, s_cfg.maxTokens, s_cfg.prompt);
    } else {
        url  = s_cfg.endpoint + "/api/chat";
        body = buildOllamaBody(s_cfg.model, b64Buf, s_cfg.maxTokens, s_cfg.prompt);
    }
    apiKey       = &s_cfg.apiKey;
    extraHeaders = &s_headers;
     ...
     * 3. HTTP POST
     * ------------------------------------------------------------------ */
    // Heap-allocate the response buffer — 4 KB on the task stack would overflow
    // the 16 KB autodoFlow task stack when combined with base64 and HTTP frames.
    ...
    if (!apiKey->empty()) {
        std::string authVal = "Bearer " + *apiKey;
    ...
    LogFile.WriteHeapInfo("LLMFallbackQueryDigit - before HTTP perform");
    esp_err_t err = esp_http_client_perform(client);

    // Free the large JSON body (~170KB) immediately after HTTP send completes.
    size_t bodyFullSize = body.size();
    std::string().swap(body);

    LogFile.WriteHeapInfo("LLMFallbackQueryDigit - after HTTP perform");
